...
const initAutocaravanasLoadMore = () => {
  const containers = document.querySelectorAll('[data-autos-grid]');
  if (!containers.length) return;

  containers.forEach((container) => {
    const loadMoreBtn = container.parentElement?.querySelector('[data-autos-load-more]');
    ...
    const filterTipo = container.dataset.filterTipo || '';
    const filterPlazas = container.dataset.filterPlazas || '';
    const filterMotor = container.dataset.filterMotor || '';
    const filterMarca = container.dataset.filterMarca || '';
    const hideSold = container.dataset.hideSold || '';
    let loading = false;
    ...
    loadMoreBtn.addEventListener('click', () => {
      ...
      if (filterPlazas) url.searchParams.set('plazas', filterPlazas);
      if (filterMotor) url.searchParams.set('motor', filterMotor);
      if (filterMarca) url.searchParams.set('marca', filterMarca);
      if (hideSold) url.searchParams.set('hide_sold', hideSold);
...
const initAutocaravanaGallery = () => {
  const gallery = document.querySelector('[data-auto-gallery]');
  const mainEl = gallery?.querySelector('.js-auto-gallery-main');
  const thumbsEl = gallery?.querySelector('.js-auto-gallery-thumbs');
  if (!gallery || !mainEl || !thumbsEl) return;
  if (typeof Swiper === 'undefined') return;
...
const initAutocaravanaTabs = () => {
  const tabLinks = document.querySelectorAll('[data-auto-tab-target]');
  const panels = document.querySelectorAll('[data-auto-tab]');
  if (!tabLinks.length || !panels.length) return;

  const setActive = (target) => {
    tabLinks.forEach((btn) => {
      const isActive = btn.dataset.autoTabTarget === target;
      btn.classList.toggle('text-[#1A2250]', isActive);
      btn.classList.toggle('font-semibold', isActive);
      btn.setAttribute('aria-pressed', isActive ? 'true' : 'false');
    });
    panels.forEach((panel) => {
      const isActive = panel.dataset.autoTab === target;
      panel.classList.toggle('hidden', !isActive);
      panel.setAttribute('aria-hidden', isActive ? 'false' : 'true');
    });
  };
  ...
  tabLinks.forEach((btn) => {
    btn.addEventListener('click', () => {
      setActive(btn.dataset.autoTabTarget);
    });
  });
};
